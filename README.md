...
This repository pins an alpha Harness release deliberately. Verify `dsh --version` before installing the profile. An older host can still render its durable `assistant/chunk` events through the compatibility fold, but it cannot provide V3-only live-stream, migration, or service behavior.
...
Only enable this where a user-question answerer exists: this app registers one,
...
| Reasoning disclosure | Width-1 spinner, live tail, settled summary, keyboard detail | Collapsible browser row |
...
- The session picker folds each candidate durable log once so rows can be sorted by last activity instead of creation time. Summaries are cached, but opening the picker against many large, previously unseen sessions can still be I/O-heavy.

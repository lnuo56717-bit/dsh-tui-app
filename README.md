...
- **Live token throughput** — while the model is streaming, the header shows `⚡ 27.5 tok/s` and refreshes it from the same 80 ms live clock. The count comes from Harness's actual provider token-boundary text, reasoning, and tool-call deltas—not character length—and terminal provider usage calibrates invisible protocol tokens before the step closes. Once the assistant message commits, the denominator freezes and the final average remains visible while idle; the next model step starts a fresh measurement.
